...
AREA_THRESHOLD = 50  # ignore tiny noise

def process_image(template_path, defect_path, save_dir, prefix):
    ...
    for cnt in contours:
        ...
        roi_name = f"{prefix}_{roi_count}.png"
        cv2.imwrite(os.path.join(save_dir, roi_name), roi)
        roi_count += 1
...
for defect_type in os.listdir(DEFECT_ROOT):
    ...
    for img_name in os.listdir(defect_dir):
        defect_img_path = os.path.join(defect_dir, img_name)
        ...
        if not os.path.exists(template_img_path):
            print(f"Template not found for {img_name}")
            continue

        prefix = img_name.replace(".jpg", "").replace(".png", "")

        roi_extracted = process_image(
            template_img_path,
            defect_img_path,
            save_dir,
            prefix
        ...
        print(f"{img_name}: {roi_extracted} ROIs extracted")

print("\n🎉 Milestone 1 automation completed successfully!")
